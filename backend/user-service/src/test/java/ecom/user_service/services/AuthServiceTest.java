...
class AuthServiceTest {
    ...
    void register_SuccessForSupportedRoles(Role role) {

        RegisterRequest request = new RegisterRequest("testuser", "test@test.com", "password", role);
        ...
        assertEquals(role, response.user().role());
    ...
    @Test
    void register_ThrowsUsernameAlreadyExistsException() {
        RegisterRequest request = new RegisterRequest("takenuser", "new@test.com", "password", Role.CLIENT);
        when(userRepository.existsByEmail(request.email())).thenReturn(false);
